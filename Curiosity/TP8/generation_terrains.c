 ...
 */
int generation_aleatoire(Terrain *T, int l, int h, float dObst) {
  ...
  int i,j;
  float prob ;
  T->tab[(int)l/2][(int)h/2]=LIBRE ;
  for(j=0;j<h;j++){
      for(i=0;i<l;i++){
          if(i==l/2 && j==h/2) continue;
          int p=rand();
          prob = ( (float)(p%101) ) /100;
          if (prob<= dObst){
            nbr_obstacle++;
            p=p%2;
            if(p==0) T->tab[i][j]=ROCHER;
            else T->tab[i][j]=EAU;
          }
