...
resultat_deplacement avancer_envt(Environnement * envt) {
  ...
  envt->x=transition( envt->x , A );
  envt->y=transition2( envt->y , A );
  // Récupérer la position devant le robot
  position_devant(envt->r, &x, &y);

  if ((x < 0) || (x >= largeur(envt->t))
      || (y < 0) || (y >= hauteur(envt->t))) {
    // Le robot est sorti
    return SORTIE;
  } else {
...
void gauche_envt(Environnement * envt) {
  envt->x=transition( envt->x , G );
  envt->y=transition2( envt->y , G );
  tourner_a_gauche(&(envt->r));
}

/* Tourner le robot à droite */
void droite_envt(Environnement * envt) {
  envt->x= transition( envt->x , D );
  envt->y=transition2( envt->y , D );
...
int mesure_envt(Environnement * envt, int d) {
  ...
  envt->x=transition( envt->x , M );
  envt->y=transition2( envt->y , M );
