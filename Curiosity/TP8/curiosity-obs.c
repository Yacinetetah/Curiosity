...
int main(int argc, char ** argv) {
  ...
  if (envt.x == ERR) {printf("la fonction «avancer» de l'environnement n'est pas toujours précédée d'une «mesure» \n");}
  else {printf("la fonction «avancer» de l'environnement est toujours précédée d'une «mesure» \n");}
  if (envt.y == ERR) { printf("Il y a une fonction «mesurer» a la fin  \n"); }
  else { printf("la fonction «mesurer» de l'environnement est toujours suivie d'une «fonction» autre que mesurer \n"); }
  return 0;
}
